	...
	{:else}
		<div id="container" style="padding-top: 20px;">
			...
			<div id="control panel">
				...
				{:else if $userInfo['friends'].includes(otherUserInfo['username']) === false && $userInfo['blocklist'].includes(otherUserInfo['username']) === false}
					<button on:click={() => { sendRequest(otherUserInfo['username']);}}>Add friend</button>
				{:else if $userInfo['friends'].includes(otherUserInfo['username'])}
					<button on:click={() => { removeFriend(otherUserInfo['username']); dispatch('message', { path: '/profile?user=' + otherUserInfo['username'] })}}>Remove friend</button>
				{/if}
				{#if $inGameUsers.includes(otherUserInfo['username']) === false && $onlineUsers.includes(otherUserInfo['username'])}
			...
			<div class="sec" id="achievements">
				<h2>Achievements</h2>
				<button on:click|preventDefault={() => dispatch('message', { path: "/achievements" })}>See all</button>
			</div>
		</div>
	{/if}
{/await}
